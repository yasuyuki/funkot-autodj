...
| Love & Joy だけ 1拍early（`bbed4d8` 後） | **曲の小節が `first_downbeat` 格子に対して途中で1拍ずれる**。アンカーは fd 格子の整数小節にしか置けないので表現できない | **未解決**（下の「小節位相のずれ」） |
...
## 小節位相のずれ（2026-08-01、未解決）

**Love & Joy は曲の途中で小節が `first_downbeat` 格子に対して1拍ずれる。**
アウトロ側のクリックは全部この1拍を引きずる。実耳報告2件がこれで説明できる:
...
真の終端は **fd + 340小節 + 1拍**。`bbed4d8` は小節数を1つ動かした＝**4拍**動かした
ので、小節内の位相は原理的に変わっていない。**直した軸が違った**。
「4拍目で鳴る」と「1拍early」は同じ物理位置を別の小節を基準に言っただけ。
...
- **`music_end_bar` のリズム判定も正しい**。最後にリズムが走っている小節は合っている。
  問題は答えを fd 格子の整数小節にしか置けないこと
- **小節内の音量プロファイル**（`examples/bar_phase_diag`）。14曲中11曲は
  「4拍目が最も静か」（小節頭直前の抜き）。Love & Joy だけ、
  **小節0〜63では4拍目が静か（格子は正しい）が、小節280〜335では1拍目が静か**
  ＝格子が1拍early。マージンは両端とも 1.4 dB
- ずれる箇所は特定できていない。小節64〜279は4拍とも同じ音量の密なセクションで、
  この統計では判定不能（`.` が並ぶ）

### 直し方が決まっていない理由

アウトロ側の小節位相を音から決める必要があるが、手持ちの統計
（`bar_phase_diag` の「最も静かな拍」）はイントロと比べて **他に2曲を動かすと言う**:
Monitoring Db +2拍、Eternal Light +3拍。どちらも実耳未検証で、ブロック列の形は
Love & Joy と同じ「前半3・後半別の値」。本当にずれているなら**ミックス側の
アウトロ整合も1〜3拍ずれている**ことになるので、統計を採用する前に耳で確かめること。

A/B 用のクリップは `examples/outro_phase_ab` で書ける（アンカーを 0〜3拍
後ろにずらした同じクリップを4本）:
...
ここは AGENTS.md の「小節 identity をキック相関の ±N拍で取らない」と正面から
衝突する領域なので、手を入れるなら [transition-phase.md](transition-phase.md) の
v10 の失敗を読み直してから。

## 残っている懸念

`lock_beat_phase` は**拍**にしか吸着せず、小節頭（mod-4）を見ない。アンカーが1拍以上
ずれた曲では、吸着先が小節頭でない拍になり得る。上の Love & Joy がまさにこれ。
