...
| フェードアウト曲だけ1小節late | 減衰の1小節目（−16 dB）を音楽と誤認（閾値 −20 dB が緩すぎた） | `TAIL_SILENCE_DB` を −14 dB へ |

実耳確認済み（クリック無し版と比較）:

- 2026-07-30 Shuki Shuki Song — 旧アンカーは小節の3拍目、新アンカー（`bbf684c` + refit）は
  小節頭。`33aac27` 後は残16小節で正しい
- 2026-07-31 Sakura Photograph — `33aac27` 時点では残15小節（1小節late）。
  閾値修正後は `fd + 432小節`（下記「フェードアウト」）
...
1. **音楽終端アンカー**（`label_session.rs::music_end_frame`、`33aac27`）
   ...
   テールの 50ms RMS 包絡で、自身の75パーセンタイルから `TAIL_SILENCE_DB`（−14 dB）以内かつ
