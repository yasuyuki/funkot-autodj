...
通常の十分な素材では4小節周期を優先する。短い素材や明示されたフェード設定で重要な
境界を4小節周期に載せられない場合は小節単位、構造・テンポの根拠が足りない場合は
非同期の短い切替へ縮退する。同期不明のまま複数小節のドラムを重ねないよう、前曲を
クリック抑制用の10 msで消音し、その後に次曲を先頭マーカーから10 msで立ち上げる。
この長さは既存のhead-only末尾処理と共通で、自然な接続の聴感受入は別途必要である。
次曲は前曲消音まで進めず、両曲を同時に減衰して長い音量の谷を作らない。通常の
同期fade設定は変えず、この縮退を小節・4小節同期の成功と数えない。
速度は動的に変えず、両曲の局所テンポ差から重ね終わりまでの
ずれを評価する。採用理由と実際の開始・entry・フェード境界はエンジンの診断で確認できる。
