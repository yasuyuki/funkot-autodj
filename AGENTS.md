...
- **下流のアプリは dev プロファイルにも `opt-level` を入れること。**
  このワークスペースの `[profile.dev.package."*"] opt-level = 2` は**下流には効かない** —
  プロファイル設定はビルド対象のワークスペースルートからしか読まれず、`funkot-core` が
  path 依存として組み込まれる側では、その**アプリ側の**設定が使われる。
  デバッグ APK は dev プロファイルなので、設定を書き忘れると `funkot-core` も
  `signalsmith-stretch` も -O0 で入る（`cc` が `OPT_LEVEL` を C++ に渡すので、
  ヘッダオンリーの DSP が最も重く効く）。7分曲・48kHz 出力のフルストレッチで
  **162s 対 7.8s（約26倍）**、デコードも 12.5s 対 0.5s を実測。
  `prepare_first_live` の 20秒ヘッドプレビューが再生し終えてもフルストレッチが
  終わらず、1曲目が数分間無音になる
