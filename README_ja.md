...
- イントロ/アウトロはつなぎ用の固定マシンリズム。イントロは8/16/32/48/64/80/96小節。
  アウトロは2つの数値を区別する: **構造境界**(曲が実際に落ちる位置)は8/16/32/64小節、
  遷移に使う**トリガ値**はその構造境界の16小節手前に固定で置くため
  24/32/48/80小節のいずれかになる。遷移の長さは16小節なので、
  **つなぎ終えた時点がちょうどアウトロの開始点**になる
  (アウトロ直前の16小節を使い、アウトロ自体には重ねない)。
  曲中のBPMは不定・可変のため、解析は曲の先頭と末尾だけを使う
...
ライブ再生中の操作:
- **Enter** — 一時停止 / 再開
- **左カーソル** — スキップ / 巻き戻し（500ms以内の連続押し）:
  - 1回: その曲の通常開始位置へトランジション
  - 2回: 前の曲の通常開始位置へトランジション
  - 3回: 前の曲のイントロへ直ちにジャンプ
- **右カーソル** — 送り（500ms以内の連続押し）:
  - 1回: 次の曲の通常開始位置へトランジション
  - 2回: 次の曲のイントロへ直ちにジャンプ
- **Ctrl+C** / kill — 終了

ナビ時のトランジションは再生位置付近のBPMを推定し、Funkot帯
...
キャッシュ形式が変わると `version` が上がり旧JSONは無効化される（現行は v14）。
...
セットアップ手順（英語）: [docs/development-setup.md](docs/development-setup.md)
