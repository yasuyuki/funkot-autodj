...
- Analysis caching works on targets that have no file locks. `File::lock` answers `Unsupported` on Android, and the cache writer treated that as a failure: every track that still needed analysis failed to prepare and could not be played, while tracks analyzed by an earlier build kept working. Writers are now serialized by a process-wide mutex on every target, and the file lock is taken on top of it wherever the OS provides one. On a target without file locks, two processes sharing one cache are no longer serialized against each other.
